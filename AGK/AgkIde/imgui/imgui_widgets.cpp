...
bool    ImGui::TabItemEx(ImGuiTabBar* tab_bar, const char* label, bool* p_open, ImGuiTabItemFlags flags, ImGuiWindow* docked_window)
{
	...
	//PE: Bug keeps getting larger and larger , never reset.
	if (tab_bar->TabsNames.size() > 3000) {
		if (tab->NameOffset == 0 || tab->NameOffset == -1 || tab->NameOffset >= tab_bar->TabsNames.size()) {
