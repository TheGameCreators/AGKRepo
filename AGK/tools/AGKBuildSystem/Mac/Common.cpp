...
int CopyFile2( const char *src, const char *dst )
{
    ...
    int result = stat( src, &properties );
    if ( result != 0 )
    {
        char err[1000]; sprintf( err, "Failed to get file properties: %s", src );
        Error( err );
        return 0;
    }
    
    int permission = properties.st_mode;
    permission &= 0777;
    permission |= 0622;
    ...
    if ( chmod( dst, permission ) != 0 )
    {
        char err[1000]; sprintf( err, "Failed to set file permissions: %s", dst );
        Error( err );
        return 0;
    }
