...
namespace AGK
{
	class cNode
	{
		private:
			...
			uint32_t m_iNodeFlags;

			void NeedsUpdate();

		protected:

			AGKUnorderedArray<cNode*> m_vChildren;
			cNode *m_pParentNode;
		...
		public:
			...
			cNode();
			virtual ~cNode();
			virtual void NeedsUpdateCallback() {} // do not do any heavy processing in inherited versions of this function, use it to set changed flags
			...
			void SetNodeRotation( float w, float x, float y, float z );
			void SetNodeScale( float x, float y, float z );
			...
			const AGKVector& posFinal();
			const AGKQuaternion& rotFinal();
			const AGKVector& scaleFinal();
