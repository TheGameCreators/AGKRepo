...
namespace AGK
{
	class cTouch
	{
		...
		public:
			...
			uint32_t GetID() { return m_iUniqueID; }
			...
			static bool GetAny( );
			static uint32_t GetCount( bool bIncludeUnknown=false );
			static cTouch* GetFirstEvent( bool bIncludeUnknown=false );
			static cTouch* GetNextEvent( );

			static void UpdateEvents();
			static cTouch* NewPoint( uint32_t iNewID );
			static cTouch* GetEvent( uint32_t iIndex );
