...
namespace AGK
{
	...
	class AGKSizedFont
	{
		...
		public:
			...
			uint32_t GetNumMainImages() { return m_iNumMainImages; }
			cImage* GetMainImage( uint32_t index ) { return (index >= m_iNumMainImages) ? 0 : m_pMainImages[ index ]; }
	...
	class AGKFont
	{
		...
		public:
			AGKFont();
			AGKFont( uint8_t* data, uint32_t compressedSize, uint32_t size );
