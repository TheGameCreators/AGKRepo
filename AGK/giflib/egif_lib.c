...
#define WRITE(_gif,_buf,_len)   \
  (((GifFilePrivateType*)_gif->Private)->Write ?    \
   ((GifFilePrivateType*)_gif->Private)->Write(_gif,(const GifByteType*)_buf,_len) :    \
...
EGifCompressLine(GifFileType * GifFile,
    ...
    while (i < LineLen) {   /* Decode LineLen items. */
        ...
        if ((NewCode = _ExistsHashTable(HashTable, (int)NewKey)) >= 0) {
            ...
            if (Private->RunningCode >= LZ_MAX_CODE) {
                ...
                _InsertHashTable(HashTable, (int)NewKey, Private->RunningCode++);
            }
        }
