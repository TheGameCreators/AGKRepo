...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	switch (message)
	{
...
#endif
			AdditionalKeyDown( (uint32_t) wParam );
			if ( wParam > 0 ) agk::KeyDown( (uint32_t) wParam );			
		...
		case WM_KEYUP:
		{
			AdditionalKeyUp( (uint32_t)wParam );
			if ( wParam > 0 ) agk::KeyUp( (uint32_t)wParam );
			break;
		}

		case WM_CHAR:
		{
			agk::CharDown( (uint32_t)wParam );
			break;
		}
                     ...
                     int       nCmdShow)
{
	...
	hWnd = CreateWin32Window( hInstance, DEVICE_WIDTH, DEVICE_HEIGHT, uString("AGK"), dwCenterWindowX, dwCenterWindowY, FULLSCREEN );
#endif
...
#else
		agk::InitGraphics( (void*) hWnd, AGK_RENDERER_MODE_NONE, 0 ); // None
#endif
