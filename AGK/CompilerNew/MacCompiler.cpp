...
{
    uint64_t now = mach_absolute_time() - *((uint64_t*)start);
	delete (uint64_t*)start;

	mach_timebase_info_data_t machTimeInfo;
	mach_timebase_info(&machTimeInfo);
	return (float) (now * 1e-9 * (machTimeInfo.numer / (double)machTimeInfo.denom));
}
...
void cFile::Init( const char* projectPath )
{
	...
	strcpy( g_szRootDir, [path UTF8String] );
	strcat( g_szRootDir, "/" );
...
bool cFile::PlatformCreatePath( uString path )
{
    ...
    char checkPath[ 1024 ];
	strcpy( checkPath, g_szWriteDir );
	if ( path.ByteAt(0) == '/' || path.ByteAt(0) == '\\' )
	{
		strcat( checkPath, path.GetStr()+1 );
	}
	else
	{
		strcat( checkPath, g_sCurrentDir );
		strcat( checkPath, path.GetStr() );
	}
    
    char *szSlash = strrchr( checkPath, '/' );
    if ( szSlash ) *szSlash = '\0';
	
    // create directory
    NSString* pWriteDir = [ [ NSString alloc ] initWithUTF8String:checkPath ];
...
bool cFile::CopyFile( const char* fromFile, const char* toFile )
{
	...
	if ( ExistsWrite( fromFile ) )
	{
		...
		NSString* pFromPath = [ [ NSString alloc ] initWithUTF8String:sPathFrom.GetStr() ];
        NSString* pToPath = [ [ NSString alloc ] initWithUTF8String:sPathTo.GetStr() ];

		[ g_pFileManager removeItemAtPath:pToPath error:nil ];
		[ g_pFileManager copyItemAtPath:pFromPath toPath:pToPath error:nil ];
...
bool cFile::RunFile( const char* filename, const char* cmdline )
{
    ...
    NSString* pPath = [ [ NSString alloc ] initWithUTF8String:filename ];
